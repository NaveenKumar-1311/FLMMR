...
for g_epoch in range(global_epochs):
          ...
          for batch_idx, (data, target) in enumerate(clients_batched[client]):
            ...
            if client == "clients_1":
                ...
                for j in range (0, 64):
                    ...
                    data[j], adv_prob, itera = SimBA(local_model.to(cuda3), org_img[j].clone().to(cuda3).unsqueeze(0), org_label[j].clone().to(cuda3).unsqueeze(0), 100, 0.7)
    ...
    print("gtanclp-ts3-emnist-simba",g_epoch, max(GTA))
    np.save('gtanclp-ts3-emnist-simba.npy', GTA)
