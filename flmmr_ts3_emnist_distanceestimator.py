...
def otattackfrombeginning(aggregiatedmodel, g_epoch):
    ...
    print(np.count_nonzero(np.equal(val1a1, val3a1)),np.count_nonzero(np.equal(val1a2, val3a2)),np.count_nonzero(np.equal(val1a3, val3a3)),np.count_nonzero(np.equal(val1a4, val3a4)),np.count_nonzero(np.equal(val1a5, val3a5)),np.count_nonzero(np.equal(val1a6, val3a6)),np.count_nonzero(np.equal(val1a7, val3a7)))
    #****************************************************************************
...
for g_epoch in range(global_epochs):
    ...
    random_clients = random.sample(client_names, k=10)
    ...
    for client in tqdm(client_names_sel):
        ...
        for epoch in range(local_epochs):
          ...
          for batch_idx, (data, target) in enumerate(clients_batched[client]):
            ...
            if client == random_clients[0]: 
                for j in range (0, 32):
                    org_probs = global_model3(data[j].clone().to(cuda3).unsqueeze(0))
                    confused_class = torch.topk(org_probs.squeeze(), 2, dim=0, largest=True, sorted=True).indices[1]
                    target[j]=confused_class
            #     org_img = data.clone()
        ...
        #     local_model = otattackfrombeginning(local_model, g_epoch)
            ...
            # global_acc, global_loss = test(local_model, test_loader)
            ...
            # for name, para in global_model.named_parameters():
            #     gd_weights.append(para.view(-1))
            # gd_weights = torch.cat(gd_weights)

            # equal_elements = torch.sum(torch.eq(bd_weights, gd_weights))
            # print(equal_elements.item(), len(bd_weights))  # 2
    ...
    print("mean:",mean_dist)
    meandistance.append(mean_dist)
    np.save('flmmr_meandistance-dpdlf.npy', meandistance)
    ...
    print("tolerance interval-dpdlf:", mean_dist-2*std_dist, mean_dist+2*std_dist)
    tintervallower.append(mean_dist-2*std_dist)
    tintervalupper.append(mean_dist+2*std_dist)
    np.save('tintervallower-dpdlf.npy', tintervallower)
    np.save('tintervalupper-dpdlf.npy', tintervalupper)
    ...
    np.save('gtanclp-ts3-emnist-dpdlf.npy', GTA)
    # torch.save(global_model.state_dict(),'surrogate_bad_emnist.pt')
#np.save('flmmr-meandistance-flmmr.npy', a)
