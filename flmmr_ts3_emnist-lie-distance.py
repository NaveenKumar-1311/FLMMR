...
def create_clients(data, num_clients, initial, alpha):
    ...
    for i in range(len(shards)):
        ...
        clients_data_list.append(len(shards[i]))

        
    return {client_names[i]: shards[i] for i in range(len(client_names))}, clients_data_list

clients, clients_data_list = create_clients(train_sampler, num_clients=10000,initial='clients', alpha=1)
...
def lie_attack(malicious_model, client_models, z):
    """
    This function aggregates client models' weights and adds random noise to create
    a new global model.
    """
    global_dict = malicious_model.state_dict()
    for k in global_dict.keys():
        weight_list = [client_models[i].state_dict()[k].float() for i in range(len(client_models))]
        avg_weights = torch.mean(torch.stack(weight_list), dim=0)
        std_weights = torch.std(torch.stack(weight_list), dim=0)
        noisy_weights = avg_weights + z * std_weights
        global_dict[k] = noisy_weights
    malicious_model.load_state_dict(global_dict)
...
for g_epoch in range(global_epochs):
    ...
    random_clients = random.sample(client_names, k=9)
    ...
    malicious_model = LeNet5(NUM_CLASSES).to(cuda3)
    malicious_model = lie_attack(malicious_model,client_models, 0.25)  

    client_models.append(malicious_model)
    bad_weights=[]


    for name, para in client_models[0].named_parameters():
        bad_weights.append(para.view(-1))
    bad_weights = torch.cat(bad_weights)

    good_weights=[]
    for i in range(len(client_models)-1):
        gd=[]
        for name, para in client_models[i+1].named_parameters():
            gd.append(para.view(-1))
        gd = torch.cat(gd)
        good_weights.append(gd)

    # distances = []
    # for gw in good_weights:
    #     distances.append(torch.dist(bad_weights, gw))
    # mean_dist = torch.mean(torch.tensor(distances))

    # print("mean:",mean_dist)
    # meandistance.append(mean_dist)
    # np.save('flmmr_meandistance-flmmr.npy', meandistance)


    mean_good_weights = torch.stack([torch.mean(gw_list, dim=0) for gw_list in good_weights]).mean(dim=0)

    # distances = []
    # for gw in good_weights:
    #     distances.append(torch.dist(bad_weights, gw))
    mean_dist = torch.dist(mean_good_weights, bad_weights)

    print("mean:",mean_dist)
    meandistance.append(mean_dist)
    np.save('flmmr_meandistance-lie.npy', meandistance)


    distances = []
    for gw in good_weights:
        distances.append(torch.dist(mean_good_weights, gw))
    mean_dist = torch.mean(torch.tensor(distances))
    std_dist = torch.std(torch.tensor(distances))

    print("tolerance interval-lie:", mean_dist-2*std_dist, mean_dist+2*std_dist)
    tintervallower.append(mean_dist-2*std_dist)
    tintervalupper.append(mean_dist+2*std_dist)
    np.save('tintervallower-lie.npy', tintervallower)
    np.save('tintervalupper-lie.npy', tintervalupper)
    ...
    print("gtanclp-ts3-emnist",g_epoch, max(GTA))
    np.save('gtanclp-ts3-emnist-liedistance.npy', GTA)
