...
def fang_attack(malicious_model, client_models):
    b=2
    ...
    for k in global_dict.keys():
        ...
        max_vector = torch.max(torch.stack(weight_list), dim=0)[0]
        min_vector = torch.min(torch.stack(weight_list), dim=0)[0]

        # Calculate wmax and wmin as described in the paper
        wmax = torch.max(max_vector)
        wmin = torch.min(min_vector)

        # Calculate deviation for Fang attack
        agg_grads = avg_weights
        deviation = torch.sign(agg_grads)


        new_shape = avg_weights.shape

# Generate random tensor between wmax and b*wmax
# new_tensor = (b - 1) * wmax * torch.rand(new_shape) + wmax

        if torch.all(deviation < 0):
            if wmax > 0:
                malicious_vec = (b - 1) * wmax * torch.rand(new_shape).to(cuda3) + wmax
            else:
                malicious_vec = new_tensor = (b - 1) * wmax * torch.rand(new_shape).to(cuda3) + wmax / b
        else:
            if wmin > 0:
                malicious_vec = (wmin / b - wmin) * torch.rand(new_shape).to(cuda3) + wmin
            else:
                malicious_vec = (b - 1) * wmin * torch.rand(new_shape).to(cuda3) + wmin
 

        global_dict[k] = malicious_vec
    malicious_model.load_state_dict(global_dict)

        # global_dict[k] = avg_weights
...
for g_epoch in range(global_epochs):
    ...
    malicious_model = fang_attack(malicious_model,client_models)  
    ...
    print("gtanclp-ts3-emnist-fang",g_epoch, max(GTA))
    np.save('gtanclp-ts3-emnist-fang.npy', GTA)
